...
train_df["CatBoost"] = y_train_pred
test_df["CatBoost"] = y_test_pred
...
avg_row.update({"Model": "CatBoost", "Set": "Average"})
metrics_df = pd.concat([metrics_df, pd.DataFrame([avg_row])], ignore_index=True)

# === 写入Excel ===
with pd.ExcelWriter("CatBoost_outputs.xlsx") as writer:
    train_df.to_excel(writer, sheet_name="Train", index=False)
    test_df.to_excel(writer, sheet_name="Test", index=False)
    metrics_df.to_excel(writer, sheet_name="Metrics", index=False)

print("✅ CatBoost 模型完成，结果已保存至 CatBoost_outputs.xlsx")
...
def plot_shap_scatter_grid(var_list, shap_values_all, X_all, colors,
    ...
    total_axes_needed = len(var_list) + 1  # +1 for图例
    ...
    # 保证 axes 数量与所需相符，多余的 remove 并裁剪
    if len(axes) < total_axes_needed:
        raise ValueError(f"子图数不足：需要 {total_axes_needed}，但仅创建了 {len(axes)} 个")
    elif len(axes) > total_axes_needed:
        # 移除多余子图，避免空白图占位
