...
train_df["LightGBM"] = y_train_pred
test_df["LightGBM"] = y_test_pred
...
avg_row.update({"Model": "LightGBM", "Set": "Average"})
metrics_df = pd.concat([metrics_df, pd.DataFrame([avg_row])], ignore_index=True)

# === 写入 Excel ===
with pd.ExcelWriter("LightGBM_outputs.xlsx") as writer:
    train_df.to_excel(writer, sheet_name="Train", index=False)
    test_df.to_excel(writer, sheet_name="Test", index=False)
    metrics_df.to_excel(writer, sheet_name="Metrics", index=False)

print("✅ LightGBM 模型完成，结果已保存至 LightGBM_outputs.xlsx")

# === SHAP 分析器 ===
shap_start = time.time()
explainer = shap.TreeExplainer(model, data=X_all, feature_perturbation="interventional")
...
for idx, (basin_id, name) in enumerate(id_to_name.items()):
    ...
    X = X_all.loc[group.index]  # 确保顺序一致
...
for basin_id, group in df.groupby("ID"):
    ...
    X = X_all.loc[group.index]  # 保持与模型训练数据一致
    shap_values = explainer(X, check_additivity=False)  # 禁用 additivity 检查

    low, high = np.percentile(shap_values.values, [1, 99])
    shap_values_clipped = np.clip(shap_values.values, low, high)
...
# === SHAP 散点图 ===
shap_values_all = explainer(X_all).values
...
def plot_shap_scatter_grid(var_list, shap_values_all, X_all, colors,
    ...
    for i, feature in enumerate(var_list):
        ...
        for basin_id in df["ID"].unique():
            ...
            ax.scatter(
                feature_vals, shap_plot_vals,
                c=subset["ID"].map(id_to_color),
                s=18, alpha=0.7, edgecolor='none', marker=marker
            )
